...
from spinifex_gnss.parse_rinex import (
    RinexHeader,
    RinexData,
    get_rinex_data as get_rinex3_data,
)
    ...
    for line_number, line in enumerate(raw_rinex_lines):
        ...
        if "RINEX VERSION / TYPE" in label:
            ...
            if len(obs_types) < n_types:
                ...
                while len(obs_types) < n_types and continue_line_num < len(
                    raw_rinex_lines
                ):
    ...
    while i < len(rinex_lines):
        ...
        if len(line) >= 26 and line[0:2].strip().isdigit():
            try:
                ...
                cur_time = (
                    Time(
                        f"{year}-{month:02d}-{day:02d}T{hour:02d}:{minute:02d}:{second:011.8f}",
                        scale="tai",
                    )
                ...
                sat_list.extend(
                    [
                        sat_line[j : j + 3].strip()
                        for j in range(0, len(sat_line), 3)
                        if sat_line[j : j + 3].strip()
                    ]
                )
                        ...
                        for j in range(0, min(5, n_types - len(obs_vals))):
                            ...
                            if field:
                                try:
                                    ...
                                    obs_vals.append(
                                        float(val_str) if val_str else np.nan
                                    )
                                except:
